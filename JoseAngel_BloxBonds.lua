--[[
    ...
    Juego: Dead Rails
    ID: 116495829188952
    Función: Buscar en TODO el mapa + Solo bonos + Recolección correcta
...
-- 🎯 FUNCIÓN ADAPTADA EXACTAMENTE PARA DEAD RAILS
local function RecolectarBonos()
    while AutoFarmActivo and task.wait(0.6) do
        ...
        -- 🔍 Búsqueda mejorada para detectar bonos en Dead Rails
        for _, Objeto in ipairs(Workspace:GetDescendants()) do
            if Objeto:IsA("BasePart") or Objeto:IsA("Model") then
                local Nombre = Objeto.Name:lower()
                -- Detecta todas las formas en que aparecen los bonos
                if Nombre:find("bond") or Nombre:find("money") or Nombre:find("cash") or Objeto:GetAttribute("IsBond") then
                    -- Si es modelo, tomamos su parte principal
                    local Parte = Objeto:IsA("Model") and Objeto.PrimaryPart or Objeto
                    if Parte and Parte:IsA("BasePart") then
                        table.insert(ListaBonos, Parte)
                    end
                end
            end
        end
        ...
        if #ListaBonos == 0 then
            ...
            StatusLabel.Text = ("Estado: %d bonos encontrados ✅"):format(#ListaBonos)
        ...
        for _, Bono in ipairs(ListaBonos) do
            ...
            -- 🎯 Formas de recoger adaptadas al juego
            if Bono:FindFirstChild("TouchInterest") then
                -- Recolección por contacto
                task.wait(0.3)
            else
                -- Llamada al evento remoto correcto que usa Dead Rails
                local Remoto = ReplicatedStorage:FindFirstChild("Remotes") 
                            or ReplicatedStorage:FindFirstChild("RemoteEvents")
                            or ReplicatedStorage

                local Recoger = Remoto:FindFirstChild("Pickup") 
                              or Remoto:FindFirstChild("Interact")
                              or Remoto:FindFirstChild("Collect")
            ...
            task.wait(0.4) -- Pequeña pausa para evitar errores
        end
    end
end
...
print("[✅] JoseAngel_Blox Bonds | Versión corregida para Dead Rails")
