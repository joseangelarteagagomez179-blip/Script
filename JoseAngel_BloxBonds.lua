...
local function Farmear()
    ...
    for _, Objeto in pairs(workspace:GetDescendants()) do
        local Prompt = Objeto:FindFirstChildWhichIsA("ProximityPrompt")
        if Prompt then
            local Distancia = (Raiz.Position - Objeto.Position).Magnitude
            if Distancia < 60 then
                Raiz.CFrame = CFrame.new(Objeto.Position + Vector3.new(0, 2, 0))
